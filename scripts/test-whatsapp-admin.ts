...
async function main() {
	...
	if (newResult === 'failed' || cancelledResult === 'failed') {
		console.log(
			'\n63031: From and To must differ — TWILIO_MESSAGING_SERVICE_SID = Twilio sandbox/prod sender; ADMIN_WHATSAPP_PHONE = your phone.\n63007: Copy the exact From from Console → Messaging → Try WhatsApp; SID/token must be the same Twilio account.',
		)
		process.exitCode = 1
	}
}
